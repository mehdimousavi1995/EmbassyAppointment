...
trait StateHelper extends StatefulBot[ClinicState] with Constants {

  def defaultGame(level: Int = 0) = GameState(level, 0, Seq.empty)

  def defaultState(userId: Long) = ClinicState(UserState(userId, startCoin, isEnteringInviteCode = false, None), defaultGame(), None)

  val db = Database(levels.toIndexedSeq)

  def withCurrentState[S, T](f: (ClinicState, Option[Level]) => T)(implicit msg: Message): T = {
